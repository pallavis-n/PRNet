...
class TrainData(object):
	...
	def getBatch(self,batch_list):
		...
		for item in batch_list:
			...
			imgs.append(im_array/255.0) # changed img_array to reflect array from above (im_array)
...
def main(args):
	...
	for epoch in xrange(epochs):
		...
		for _ in xrange(int(math.ceil(1.0*data.num_data/batch_size))):
			...
			print('iters:/epoch:%d,learning rate:%f,loss:%f'%(epoch,learning_rate,loss_res)) # changed learn_rate to learning_rate and got rid of iters argument (%d)
